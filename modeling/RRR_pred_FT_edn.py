...
def mint(n, train_X, test_X, train_y, test_y, device, batch, ndf='RRR'):
    ...
    # Export the result
    final_results.to_csv('inference_MINT_pred_' + ndf + '_nandes_'+ str(batch) + '_torch.csv', encoding='utf-8', index=False)
...
if __name__ == "__main__":
    ...
    X_train = np.load('embeddings_ft_name.npy')
    y_train = pd.read_csv('target_all_clean.csv')
    # number of clusters
    num_sc = len(set(y_train['sc']))
    # Run baseline for classifying the soft cluster pseudo-labels and get the predicted food category for the inference
    ...
    for b in tqdm(range(1,12)):
        ...
        y_test = pd.read_csv('inference_nandes_' + str(b) + '.csv',
                             low_memory=False, lineterminator='\n')
        y_test = y_test[['id','name']]
        X_test = np.load('inference_nandes_FT_embedding_' + str(b) + '.npy')
        # Predict food category for the test dataset
