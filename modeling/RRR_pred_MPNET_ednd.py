...
def base_mlp(train_X, train_y, device, ndf='RRR'):
    ...
    torch.save(model.state_dict(), ndf + '_weights_MPNET_ED.pth')
    # del model
    # gc.collect()
    # torch.cuda.empty_cache()
...
def mint(n, train_X, test_X, train_y, test_y, device, batch, ndf='RRR'):
    ...
    true_test, pred_test, ci_test = finetune(n, train_X, test_X, train_dict, test_y, ndf,
                                             ndf + '_weights_MPNET_ED.pth', device)
    ...
    # Export the result
    final_results.to_csv('inference_MINT_pred_' + ndf + '_des_'+ str(batch) + '_torch.csv', encoding='utf-8', index=False)
...
if __name__ == "__main__":
    ...
    # Use edamam's pseudo-labels from ChatGPT to predict the AMDD label for the inferenc datset
    # Read all edamam dataset as training and test it on inference
    X_train = np.load('embeddings_mpnet_gptdes.npy')
    y_train = pd.read_csv('target_all_clean.csv')
    # number of clusters
    num_sc = len(set(y_train['sc']))
    # Run baseline for classifying the soft cluster pseudo-labels and get the predicted food category for the inference df
    ...
    sc_CLS_model.eval()
    # Run in batch
    for b in tqdm(range(1,12)):
        # Read inference dataset
        y_test = pd.read_csv('inference_des_' + str(b) + '.csv', low_memory=False, lineterminator='\n')
        y_test = y_test[['id','name']]
        X_test = np.load('inference_des_MPNET_embedding_' + str(b) + '.npy')
        # Predict food category for the test dataset
        X_test = torch.tensor(X_test, dtype=torch.float32).to(gpu)
        y_pred_target = sc_CLS_model(X_test)
        ...
        # y_test.to_csv('inference_des_wfc_testdf_' + str(b) +  '.csv',index=False)
        # Run MINT
        mint(num_sc, X_train, X_test, y_train, y_test, gpu, b)
